    ...
    return(
      <TouchableOpacity onPress={this.handleContainerPress} style={{ backgroundColor: "white", flex: 1 }}> 
        {this.props.images.length !== 0?(
          <FastImage 
            source={{ uri: firstImage.image.uri }} 
            resizeMode="cover"
            style={{ height: 200, ...styles.singleImage }}/>
        ):<View/>}

        <View style={{ flex: 1, flexDirection: "row", justifyContent: "space-evenly" }}>
          {this.props.images.map((item, index) => {
            if((index > 0 && index < 4) || index === 4 && remainingImageCount === 0) {
              return (
                <FastImage 
                  key={index} 
                  resizeMode="cover"
                  source={{ uri: item.image.uri,  }} 
                  style={{ height: (window.width/4), flex: 1, alignItems: "stretch" }}/>
              )
            }else if(index === 4 && remainingImageCount > 0) return (
              <View key={index} style={{ alignSelf: "stretch", flex: 1, height: (window.width/4) }}>
                <FastImage source={{ uri: item.image.uri }} style={{ alignSelf: "stretch", flex: 1 }} resizeMode="cover"/>
...
const styles = StyleSheet.create({
  singleImage: { alignItems: "stretch" }
})